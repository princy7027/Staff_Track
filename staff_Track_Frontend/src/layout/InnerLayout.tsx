import { AioutlineMenu } from "@/common/icons";
...
const InnerLayout = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const currentTimeIST = new Date().getTime() + 5.5 * 60 * 60 * 1000;
    const currentHourIST = new Date(currentTimeIST).getUTCHours();
    if (currentHourIST >= 0 && currentHourIST < 12) {
      setGreeting("Good Morning");
    } else if (currentHourIST >= 12 && currentHourIST < 17) {
        ...
        {isOpen && (
          <div className="fixed top-0 left-0 z-40 w-full h-full bg-black/50">
            <div
              ...
              className=" w-64 h-screen p-4 relative overflow-y-auto transition-transform  bg-white dark:bg-gray-800"
              tabIndex={-1}
